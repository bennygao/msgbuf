...
void initialize_grammar(void)
{
    nitems = 4;
    maxitems = 300;
    pitem = (bucket **) allocate(maxitems*sizeof(bucket *));
    if (pitem == 0) no_space();
    pitem[0] = 0;
    pitem[1] = 0;
    pitem[2] = 0;
    ...
    nrules = 3;
    maxrules = 100;
    plhs = (bucket **) allocate(maxrules*sizeof(bucket *));
    if (plhs == 0) no_space();
    plhs[0] = 0;
    plhs[1] = 0;
    plhs[2] = 0;
    rprec = (short *) allocate(maxrules*sizeof(short));
    if (rprec == 0) no_space();
    rprec[0] = 0;
    rprec[1] = 0;
    rprec[2] = 0;
    rassoc = (char *) allocate(maxrules*sizeof(char));
...
int i;
    ...
    if (!last_was_action && plhs[nrules]->tag)
    {
		...
		/* BUG!!!
		 * The original code like following.
		 * -------------------------------------------------------------
		 * if (pitem[i+1] == 0 || pitem[i+1]->tag != plhs[nrules]->tag)
		 * -------------------------------------------------------------
		 * under some compiler like MSVC++, the second condition expression
		 * pitem[i+1]->tag != ... will be excuted before the first expression.
		 * When pitem[i+1] is NULL, the result is reference its tag attribute first,
		 * cause memory access violation.
		 */
		if (pitem[i+1] == 0) {
			default_action_warning();
		} else if (pitem[i+1]->tag != plhs[nrules]->tag) {
				default_action_warning();
		}
    }
...
char *p, *s, *t;
    ...
    name_pool = allocate(name_pool_size);
...
int i, j, k, n;
    ...
    symbol_name = (char **) allocate(nsyms*sizeof(char *));
    if (symbol_name == 0) no_space();
    symbol_value = (short *) allocate(nsyms*sizeof(short));
    if (symbol_value == 0) no_space();
    symbol_prec = (short *) allocate(nsyms*sizeof(short));
    if (symbol_prec == 0) no_space();
    symbol_assoc = allocate(nsyms);
    if (symbol_assoc == 0) no_space();

    v = (bucket **) allocate(nsyms*sizeof(bucket *));
    if (v == 0) no_space();
...
int assoc, prec;
    ...
    ritem = (short *) allocate(nitems*sizeof(short));
    if (ritem == 0) no_space();
    rlhs = (short *) allocate(nrules*sizeof(short));
    if (rlhs == 0) no_space();
    rrhs = (short *) allocate((nrules+1)*sizeof(short));
    if (rrhs == 0) no_space();
