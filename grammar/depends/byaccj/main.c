...
FILE *init_file;	/*	data variable initialization code file */
...
void done(int k)
{
  ...
  if (init_file) { fclose(init_file); unlink(init_file_name); }
  exit(k);
}
...
char *tmpdir;
    ...
    action_file_name = allocate(i);
    if (action_file_name == 0) no_space();
    text_file_name = allocate(i);
    if (text_file_name == 0) no_space();
    union_file_name = allocate(i);
    if (union_file_name == 0) no_space();
	init_file_name = allocate(i);
	if (init_file_name == 0) no_space();

    strcpy(action_file_name, tmpdir);
    strcpy(text_file_name, tmpdir);
    strcpy(union_file_name, tmpdir);
	strcpy(init_file_name, tmpdir);
	...
	strcpy(init_file_name + len, temp_form);

    action_file_name[len + 5] = 'a';
    text_file_name[len + 5] = 't';
    union_file_name[len + 5] = 'u';
	init_file_name[len + 5] = 'j';

#if !defined(__WIN32__)
    mktemp(action_file_name);
    mktemp(text_file_name);
    mktemp(union_file_name);
	mktemp(init_file_name);
#else
	_mktemp(action_file_name);
    _mktemp(text_file_name);
    _mktemp(union_file_name);
	_mktemp(init_file_name);
#endif
    ...
    if (jflag)/*rwj*/
      ...
      output_file_name = allocate(len + 6);/*rwj for '.java\0' */
      ...
      output_file_name = allocate(len + 7);
      if (output_file_name == 0) no_space();
    ...
    if (dflag)
    {
	defines_file_name = allocate(len + 7);
    ...
    if (vflag)
    {
	verbose_file_name = allocate(len + 8);
...
void open_files(void)
{
  ...
  if (jflag) {
	  if ((init_file = fopen(init_file_name, "w")) == NULL)
		  open_error(init_file_name);
  }
