...
void output(void)
{
    ...
    if (jflag) {
		output_java_init_code_header();
		output_java_defines();
		output_java_rule_data();
		output_java_yydefred();
	} else {
		output_defines();
		output_rule_data();
		output_yydefred();
    ...
    if (jflag) {   /*rwj*/
      write_section(jbody1);
	  output_new_yyval();
	  write_section(jbody2);
    } else
      write_section(body);
...
char **symnam, *s;
    ...
    symnam = (char **) allocate((max+1)*sizeof(char *));
...
FILE *f;
  if (jflag)/*rwj*/
    ...
    fprintf(code_file,"//#### end semantic value section ####\n");
      ...
      {
    fprintf(code_file,"\n\n//########## SEMANTIC VALUES ##########\n");
    fprintf(code_file,"//public class %sVal is defined in %sVal.java\n\n\n",
                        jclass_name,jclass_name);
    sprintf(filenam,"%sVal.java",jclass_name);
    f=fopen(filenam,"w");
    if (!f)
      return;
    fprintf(f,"//#############################################\n");
...
//----------------------------------------------------------------------
// Following functions are added to support output Java code.
//----------------------------------------------------------------------

void output_java_base(void)
{
	int i, j;
	int ncount = 0;
	int innerClassNum = 0;
    
    fprintf(output_file, "static short[] yysindex = null;\n");
	for (i = 0; i < nstates; ++i) {
		if ((i == 0) || (i % MAX_DATA_NUM == 0)) {
			...
			fprintf(output_file, "class YYSINDEX%d {\n", 
				innerClassNum++);
			fprintf(output_file, "  final short yysindex[] = {");
			ncount = 0;
		}
	...
	output_java_init_code(
		"YYSINDEX", "yysindex", "short", innerClassNum);


	innerClassNum = 0;
	fprintf(output_file, "static short[] yyrindex = null;");
	for (i = nstates, j = 0; i < 2 * nstates; i++, j++) {
		if ((j == 0) || (j % MAX_DATA_NUM == 0)) {
			if (j > 0) {
				fprintf(output_file, "\n  };\n}\n\n");
			}

			fprintf(output_file, "class YYRINDEX%d {\n", 
				innerClassNum++);
			fprintf(output_file, "  final short yyrindex[] = {");
			ncount = 0;
		}
	...
	fprintf(output_file, "\n  };\n}\n\n");
	output_java_init_code(
		"YYRINDEX", "yyrindex", "short", innerClassNum);

	innerClassNum = 0;
	fprintf(output_file, "static short[] yygindex = null;\n");
	for (i = 2 * nstates, j = 0; i < nvectors - 1; i++, j++) {
		if ((j == 0) || (j % MAX_DATA_NUM == 0)) {
			if (j > 0) {
				fprintf(output_file, "\n  };\n}\n\n");
			}

			fprintf(output_file, "class YYGINDEX%d {\n", 
				innerClassNum++);
			fprintf(output_file, "  final short yygindex[] = {");
			ncount = 0;
		}
	...
	fprintf(output_file, "\n  };\n}\n\n");
	output_java_init_code(
		"YYGINDEX", "yygindex", "short", innerClassNum);	    

    FREE(base);
	fflush(output_file);
}

void output_java_table(void)
{
	int i;
	int ncount = 0;
	int innerClassNum = 0;
	...
	fprintf(code_file, "final static int YYTABLESIZE=%d;\n", high);
	fprintf(output_file, "static short[] yytable = null;\n");
	for (i = 0; i <= high; ++i) {
		if ((i == 0) || (i % MAX_DATA_NUM == 0)) {
			if (i > 0) {
				fprintf(output_file, "\n  };\n}\n\n");
			...
			fprintf(output_file, "class YYTABLE%d {\n", 
				innerClassNum++);
			fprintf(output_file, "  final short yytable[] = {");
			ncount = 0;
		}
	...
	fprintf(output_file, "\n  };\n}\n\n");
	output_java_init_code(
		"YYTABLE", "yytable", "short", innerClassNum);

    fflush(output_file);
    FREE(table);
}

void output_java_check(void)
{
    int i;
	int ncount = 0;
	int innerClassNum = 0;
	...
	fprintf(output_file, "static short[] yycheck = null;\n");
	for (i = 0; i <= high; ++i) {
		if ((i == 0) || (i % MAX_DATA_NUM == 0)) {
			if (i > 0) {
				fprintf(output_file, "\n  };\n}\n\n");
			...
			fprintf(output_file, "class YYCHECK%d {\n", 
				innerClassNum++);
			fprintf(output_file, "  final short yycheck[] = {");
			ncount = 0;
		}
	...
	fprintf(output_file, "\n  };\n}\n\n");
	output_java_init_code(
		"YYCHECK", "yycheck", "short", innerClassNum);
