...
final public class Emitter {
	// UTF BOM
	private final static String BOM_UTF_16BE = "\ufeff";
	private final static String BOM_UTF_16LE = "\ufffe";
	private final static String BOM_UTF_8 = "\uefbbbf";
	...
	public Emitter(File inputFile, LexParse parser, DFA dfa) throws IOException {
		...
		if (Options.encoding == null) {
			...
			writer = new OutputStreamWriter(new FileOutputStream(outputFile), Options.encoding);
		}
	 ...
	 * @param path
	 *            the path where to construct the file
	 * @param input
	...
	private void println(String line) {
		line = line.replace(BOM_UTF_16BE, "");
		line = line.replace(BOM_UTF_16LE, "");
		line = line.replace(BOM_UTF_8, "");
		out.println(line);
	}
...
//		String contents = "/* The following code was generated by JFlex "
//				+ Main.version + " on " + date + " */";
//		println(contents);
//		println();
	}
	...
	private void emitUserCode() {
		if (scanner.userCode.length() > 0) {
			String userCode = scanner.userCode.toString();
			println(userCode);
		}
	}
